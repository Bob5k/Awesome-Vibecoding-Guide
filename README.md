...
Define the feature and outcomes (write a brief PRD).



Create specifications per feature (OpenSpec or your preferred format).



Set up your stack: Zed, GLM, Context7 MCP, DevTools MCP.



Load only relevant files into context; keep it lean.



Implement tasks one by one with an AI agent; review after each.



Test using DevTools MCP first, then manual checks.



Commit per finished feature; review before merge.
...
Pro tip: It’s more efficient to tell the LLM “use MCP” instead of writing a long debugging prompt. Pro tip 2: For web development, it can even perform its own tests using Chrome DevTools MCP when prompted correctly.
...
Community pro tip: If you have 1M context (Claude), you can fill it but compress around ~200k. The AI won’t feel end-of-context pressure and won’t trigger “rush-to-finish” behavior.
...
Ideation—dialog with AI about the idea



Product Requirements Document (PRD)—detailed feature description



Specifications—split into features



Task Breakdown—each feature broken into tasks

Output: /docs/ ├── prd.md ├── architecture.md ├── features/ │ ├── feature-1-spec.md │ ├── feature-1-tasks.md │ ├── feature-2-spec.md │ └── feature-2-tasks.md └── tech-stack.md
...
Workflow: Open the project in Zed

Load the feature spec into context

One feature at a time: a. Read the tasks b. Agent implements task by task c. After each task: manual review d. After the feature: comprehensive review
...
Example session: Session 1: User Authentication Feature
...
Don’t: "Doesn’t work, fix it" "Still doesn’t work, fix pls" (x100)

Do: Analyze the bug (manually or via MCP)
...
Use MCP to gather precise logs



Analyze them before the next prompt



You may need to change approach, not just code
...
First pass—Claude 3.5 / GPT-4o review of the entire feature codebase
...
Refactoring—GLM implements suggestions



Second review—ensure everything works
...
Automated deployment: Setup CI/CD git push → automatic deploy
